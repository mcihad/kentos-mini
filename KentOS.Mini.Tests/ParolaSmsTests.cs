...
public class ParolaSmsTests
{
    ...
    private static string KaynakKoku()
    {
        ...
        while (dizin is not null && !Directory.Exists(Path.Combine(dizin, "KentOS.Mini.Web")))
        {
    ...
    {
        var kaynak = Oku(Path.Combine("KentOS.Mini.Web", "Services", "V2", "YonetimServisi.cs"));
    ...
    {
        var kaynak = Oku(Path.Combine("KentOS.Mini.Web", "Services", "FirebaseWorker.cs"));
    ...
    {
        var kaynak = Oku(Path.Combine("KentOS.Mini.Application", "Models", "Message.cs"));
